<?php
/**
 * Admin Dashboard (Logs) for SFF AI Takeaway
 */
...
add_action( 'admin_menu', 'sff_ai_takeaway_dashboard_menu', 11 );
function sff_ai_takeaway_dashboard_menu() {
    add_submenu_page(
        'sff-ai-takeaway',
        'Interaction Logs',
        'Interaction Logs',
        'manage_options',
        'sff-ai-takeaway-logs',
        'sff_ai_takeaway_logs_page'
    );
}

function sff_ai_takeaway_logs_page() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'sff_ai_logs';

    // Handle clearing logs
    if ( isset( $_POST['sff_ai_clear_logs'] ) && check_admin_referer( 'sff_ai_clear_logs_nonce' ) ) {
        $wpdb->query( "TRUNCATE TABLE $table_name" );
        echo '<div class="updated"><p>All logs cleared successfully.</p></div>';
    }

    // Handle individual row deletion
    if ( isset( $_GET['delete_log'] ) && check_admin_referer( 'sff_ai_delete_log_' . $_GET['delete_log'] ) ) {
    ...
    if ( isset( $_POST['sff_ai_delete_date'] ) && check_admin_referer( 'sff_ai_bulk_logs_nonce' ) ) {
    ...
    if ( isset( $_POST['sff_ai_prune_logs'] ) && check_admin_referer( 'sff_ai_bulk_logs_nonce' ) ) {
        sff_ai_perform_log_cleanup();
    ...
    ?>
    <div class="wrap">
        <h1>SFF AI Takeaway - Interaction Logs</h1>
        ...
        <div class="tablenav top" style="display: flex; justify-content: space-between; align-items: center; background: #fff; padding: 10px; border: 1px solid #ccd0d4; border-radius: 4px; margin-bottom: 20px;">
            <div class="alignleft actions">
                <form method="get" action="" style="display: flex; align-items: center; gap: 10px;">
                    <input type="hidden" name="page" value="sff-ai-takeaway-logs">
                    ...
                    <?php if($filter_date): ?>
                        <a href="admin.php?page=sff-ai-takeaway-logs" class="button">Clear</a>
                    <?php endif; ?>
                </form>
            </div>
            ...
            <div class="alignright actions" style="display: flex; gap: 10px;">
                <?php if($filter_date): ?>
                    <form method="post" action="" onsubmit="return confirm('Delete ALL logs for <?php echo esc_attr($filter_date); ?>?');">
                        <?php wp_nonce_field( 'sff_ai_bulk_logs_nonce' ); ?>
                        <input type="hidden" name="filter_date" value="<?php echo esc_attr($filter_date); ?>">
                        <input type="submit" name="sff_ai_delete_date" class="button" style="color:#d63638; border-color:#d63638;" value="Delete All for This Date">
                    </form>
                <?php endif; ?>

                <form method="post" action="" onsubmit="return confirm('Prune logs older than configured retention days?');">
                    <?php wp_nonce_field( 'sff_ai_bulk_logs_nonce' ); ?>
                    <input type="submit" name="sff_ai_prune_logs" class="button" value="Run Cleanup (Prune)">
                </form>

                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL logs? This cannot be undone.');">
                    <?php wp_nonce_field( 'sff_ai_clear_logs_nonce' ); ?>
                    <input type="submit" name="sff_ai_clear_logs" class="button button-link-delete" value="Clear Everything" style="color:#d63638;">
                </form>
            </div>
        </div>
            ...
            <tbody>
                <?php if ( $logs ) : ?>
                    <?php foreach ( $logs as $log ) : 
                        ...
                        $delete_url = wp_nonce_url( admin_url( 'admin.php?page=sff-ai-takeaway-logs&delete_log=' . $log->id ), 'sff_ai_delete_log_' . $log->id );
                        ?>
                        <tr>
